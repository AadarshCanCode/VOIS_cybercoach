...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
    ({ className = '', label, error, leftIcon, rightIcon, type, cyber, showPasswordToggle = true, ...props }, ref) => {
        ...
        return (
            <div className="w-full space-y-1.5 focus-within:z-20">
                {label && (
                    <label className={`block ml-1 ${cyber
                ...
                <div className="relative flex items-center">
                    ...
                    <input
                        type={inputType}
                        className={`flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${leftIcon ? 'pl-10' : ''
                ...
                {error && (
                    <p className={`text-sm ${cyber ? 'font-mono text-[10px] uppercase tracking-widest text-red-500' : 'text-destructive'}`}>
                        {error}
                    </p>
                )}
            </div>
